    ...
    public function handle()
    {
        ...
        if ($this->generateClass($force)){
            ...
            if ($this->strReplaceInFile(
                resource_path('views/vendor/brackets/admin-ui/admin/layout/sidebar.blade.php'),
                '|url\(\'' . config('admin-generator.admin_url_prefix') . $this->resource.'\'\)|',
                "{{-- Do not delete me :) I'm used for auto-generation menu items --}}",
                "<li class=\"nav-item\"><a class=\"nav-link\" href=\"{{ url('" . config('admin-generator.admin_url_prefix') . $this->resource."') }}\"><i class=\"nav-icon ".$icon."\"></i> {{ trans('admin.".$this->modelLangFormat.".title') }}</a></li>".PHP_EOL."           {{-- Do not delete me :) I'm used for auto-generation menu items --}}"
            )) {
