...
{{'@'}}section('body')
    ...
    <{{ $modelJSName }}-listing
        ...
        :url="'{{'{{'}} url('{{ config('admin-generator.admin_url_prefix') }}{{ $resource }}') }}'"
        inline-template>
...
@if($export)
                        <a class="btn btn-primary btn-sm pull-right m-b-0 ml-2" href="{{'{{'}} url('{{ config('admin-generator.admin_url_prefix') }}{{ $resource }}/export') }}" role="button"><i class="fa fa-file-excel-o"></i>&nbsp; {{'{{'}} trans('admin.{{ $modelLangFormat }}.actions.export') }}</a>
@endif
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0" href="{{'{{'}} url('{{ config('admin-generator.admin_url_prefix') }}{{ $resource }}/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{'{{'}} trans('admin.{{ $modelLangFormat }}.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
	                    ...
	                    <div class="no-items-found" v-if="!collection.length > 0">
                            ...
                            <a class="btn btn-primary btn-spinner" href="{{'{{'}} url('{{ config('admin-generator.admin_url_prefix') }}{{ $resource }}/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; @{{ trans('brackets/admin-ui::admin.btn.new') }} {{ $modelBaseName }}</a>
	                    </div>
                    </div>
                </div>
            </div>
